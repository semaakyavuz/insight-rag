...
function sourcesToLabels(sources: QuerySource[] | null | undefined) {
    ...
    try {
      ...
      for (let i = 0; i < detail.messages.length; i += 2) {
        ...
        loaded.push({
          ...
          sources: sourcesToLabels(assistantMessage.sources),
        });
      }

      setAnswers(loaded.reverse());
      setConversationId(id);
    } catch {
      // Keep the current view if the conversation failed to load.
    }
  }, []);
      ...
      setAnswers((prev) => [
        {
          ...
          sources: sourcesToLabels(result.sources),
        },
        ...prev,
      ]);
  ...
  return (
    <div className="flex min-h-full">
      ...
      <button
        ...
        aria-label="Konuşma geçmişini aç/kapat"
        className="fixed top-[calc(3.5rem+0.75rem)] left-3 z-40 rounded-md border border-border bg-background/80 p-2 shadow-sm backdrop-blur md:hidden"
      >
        {isMobileSidebarOpen ? (
          <X className="size-5" />
        ) : (
